# You want to water n plants in your garden with a watering can. The plants are arranged in a row and are labeled from 0 to n - 1 from 
# left to right where the ith plant is located at x = i. There is a river at x = -1 that you can refill your watering can at.
...
class Solution:
    def wateringPlants(self, plants: List[int], capacity: int) -> int:
        ...
        for i in range(len(plants)):
            if runCapacity >= plants[i]:
                countSteps += 1
                runCapacity -= plants[i]
            else:
                countSteps += (i + i + 1)
                runCapacity = capacity - plants[i]
