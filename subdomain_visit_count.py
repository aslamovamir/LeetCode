...
# Given an array of count-paired domains cpdomains, return an array of the count-paired domains of each subdomain in the input. You 
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            seperate = cpdomain.split(' ')
            frequency = int(seperate[0])
            domains = seperate[1].split('.')
            for i in range(len(domains)):
                considered = '.'.join(domains[i:])
                if considered not in tracker:
                    tracker[considered] = frequency
                else:
                    tracker[considered] += frequency
        
        results = []
        for key, value in tracker.items():
            results.append(' '.join([str(value), key]))
